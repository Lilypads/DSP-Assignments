...
for Case in range (2):
    ...
    threshT = int(1/(3.66/fs))    #3.66beat/s is the maximum heart rate(220bpm)
    
    for i in range(len(hr)):
        if hr[i] == 1:
            peakCounter+=1
            for n in range(int(threshT)):    #fix the impossible detected peak to 0
...
class HR_filter:
    ...
    def realtimeHR(self,data,fs):        
        ...
        matchresult = matchresult*matchresult
        threshold = 0.00000000001               #observed by eye from graph
...
data2 = loadtxt("shorteint.dat")  
fs=250
mhr = np.zeros(len(data))
mhr2 = np.zeros(len(data2))
hrfilt = HR_filter()                            #instantiate the HR_filter
for i in range(len(data)):
    mhr[i] = hrfilt.realtimeHR(data[i],fs)
for i in range(len(data2)):
    mhr2[i] = hrfilt.realtimeHR(data2[i],fs)
    
pyplot.figure(17)
pyplot.plot(np.linspace(0,len(data)/fs,len(data)),mhr)
pyplot.title('Momentary Heart Rate Plot from ECG.dat')
pyplot.xlabel('Time(s)')
pyplot.ylabel('Beats/Minute')
#pyplot.savefig('hr_fig17.eps', format='eps')

pyplot.figure(18)
pyplot.plot(np.linspace(0,len(data2)/fs,len(data2)),mhr2)
pyplot.title('Momentary Heart Rate Plot from einthoven II')
pyplot.xlabel('Time(s)')
pyplot.ylabel('Beats/Minute')
#pyplot.savefig('hr_fig18.eps', format='eps')
