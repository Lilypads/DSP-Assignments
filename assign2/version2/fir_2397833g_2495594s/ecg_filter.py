...
Coeff50[0:int(M/2)] =  W50[int(M/2):M]
Coeff50[int(M/2):M] =  W50[0:int(M/2)]
...
for line in file1: 
    ...
    ecg1 = line.strip()
    #print(ecg1)
    intermediate = Filter50.dofilter(ecg1)
    filterecg[count] = FilterDC.dofilter(intermediate)
    
for i in range(len(einthoven_ii)): 
    intermediate = Filter50.dofilter(einthoven_ii[i])
